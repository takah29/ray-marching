...
float distance_estimate(in RecursiveTetrahedron rt, in vec3 p) {
    vec4 z = vec4(p, 1.0);
...
float distance_estimate(in MengerSponge ms, in vec3 p) {
    vec4 z = vec4(p, 1.0);
...
float distance_estimate(in HexTiling ht, in vec3 p) {
...
float distance_estimate_mb_simple(in Mandelbulb mb, in vec3 p) {
    vec3 z = p;
    float dr = 1.0;
    float r;
...
float distance_estimate_mandelbulb(in Mandelbulb mb, in vec3 p, out vec4 res_color) {
    vec3 w = p;
    float m = dot(w, w);
